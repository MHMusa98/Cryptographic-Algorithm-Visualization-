...
function selectAlgorithm(name) {
	...
	if(name == 'Caesar'){
		...
		decryptButton.addEventListener('click', caesarDecrypt);
	}
...
function caesarEncrypt () {
  ...
  for (let i = 0; i < message.length; i++) {
    ...
    result += String.fromCharCode(charCode);
		...
		document.getElementById('showEncryptedText').innerHTML = result;
	}
  ...
  for (let i = 0; i < message.length; i++) {
    ...
    result += String.fromCharCode(charCode);
		...
		document.getElementById('showDecryptedText').innerHTML = result;
	}
